...
/*Some Static Colors------------------------------*/
static RGB_Color_TypeDef RGB_Color[] = {{0, 0, 0},{255, 0, 0},{127, 106, 0},{127, 216, 0},{0, 255, 0},{0, 255, 255},{0, 0, 255},{238, 130, 238},{0, 0, 0},{255, 255, 255},{255, 0, 220}};
#define TRANSPARENT  RGB_Color[0] //透明
#define RED          RGB_Color[1]
#define ORANGE       RGB_Color[2]
#define YELLOW       RGB_Color[3]
#define GREEN        RGB_Color[4]
#define CYAN         RGB_Color[5]
#define BLUE         RGB_Color[6]
#define PURPLE       RGB_Color[7]
#define BLACK        RGB_Color[8]
#define WHITE        RGB_Color[9]
#define MAGENTA      RGB_Color[10]
...
/*
功能：最后一行装在24个0，输出24个周期占空比为0的PWM波，作为最后reset延时，这里总时长为24*1.2=30us
> 24us(要求大于24us)
*/
...
void RGB_SendArray(void) {
    HAL_TIM_PWM_Start_DMA(&htim1, TIM_CHANNEL_4, (uint32_t *)Pixel_Buf,(LED_NUM + 1) * 24);
}
...
// u32的流水灯
void RGB_Flowing(u32 data,u8 colorKey){
    uint32_t mask = 0b10000000000000000000000000000000; // 位位置，从最高位开始
    for (uint8_t k = 0; k < 30; ++k) {
        if (data & mask) {
            // printf("1");
            RGB_SetColor(k, RGB_Color[colorKey]);
        } else {
            // printf("0");
            RGB_SetColor(k, TRANSPARENT);
        }
        mask = mask >> 1U;
    }
    Reset_Load();
...
static void rainbow(uint8_t wait) {
    ...
    uint32_t flag = 0;
    if (next_time < wait) {
        if ((uint64_t)timestamp + wait - next_time > 0)flag = 1;
    }else if(timestamp > next_time) { flag = 1; }
